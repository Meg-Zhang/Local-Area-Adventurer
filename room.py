...
def signal_handler(sig, frame):
    ...
    # DEREGISTER room from discovery service
    sentence = "DEREGISTER " + name.lower()
    room_socket.sendto(sentence.encode(), ("", DISCOVERY_PORT))
    sys.exit(0)
...
def process_message(message, addr, room_socket):
    ...
    # TODO Discovery service messages
    if words[0] == 'OK':
        pass # nothing needs to be done if the message is OK.

    if words[0] == "NOTOK" and message != "NOTOK - Room does not exist":
        print(message)
        exit(0)
    ...
    # If player is leaving the server. remove them from the list of players and notify other people in the room.
        ...
        print(message) #  print leave message to room console
        for player in players:
            client_addr = client_search(player)
            room_socket.sendto(message.encode(), client_addr)
    ...
    # TODO disconnect command if discovery service exits
    elif (words[0] == 'disconnect'):
        print('Discovery service has shut down, shutting down ...')
        players = client_list_except_player('')
        message = 'disconnect'
        for player in players:
            client_addr = client_search(player)
...
def main():
    ...
    port = 0  # setting port to 0 will have an available port be allocated to this room.
    name = args.name
    description = args.description
    items = args.item

    # Set up connections to other rooms.
    # TODO Make these rooms rely on the lookup service.
    if args.n != "":
    ...
    port = room_socket.getsockname()  # After binding the socket, get the port number assigned.

    # TODO Register Room.
    registration_message = "REGISTER " + name
    room_socket.sendto(registration_message.encode(), ('', DISCOVERY_PORT))
